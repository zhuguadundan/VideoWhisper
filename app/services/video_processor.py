...
from app.services.video_downloader import DownloadCancelled, VideoDownloader
...
class VideoProcessor:
    ...
    def __init__(self):
        ...
        self.long_audio_threshold = int(
            proc_cfg.get("long_audio_threshold_seconds", 300)
        )
    ...
    def _sanitize_filename(self, filename: str) -> str:
        """统一文件名清洗，复用公共实现"""
        return utils_sanitize_filename(
    ...
    def create_upload_task(
        self, original_filename: str, file_size: int, file_type: str, mime_type: str
    ) -> str:
    ...
    def complete_upload_task(
        self, task_id: str, file_path: str, file_duration: float = 0
    ):
        """完成上传任务，设置文件路径"""
        with self._lock:
            task = self.tasks.get(task_id)
    ...
    def _step_get_video_info(
        ...
        url = (
            (video_info_dict or {}).get("url")
            or (video_info_dict or {}).get("webpage_url")
            or task.video_url
        )
        try:
    ...
    def _step_download_audio(self, task_id: str, task: ProcessingTask) -> str:
        ...
        try:
            ...
            self.logger.debug(
                f"[DEBUG] download_audio_only完成，返回路径: {audio_path}"
            )
    ...
    def download_video_only(self, task_id: str) -> ProcessingTask:
        """仅下载视频到 output/，并更新任务进度。

        用于 /api/downloads 创建的下载任务。
        """
        ...
        try:
            ...
            self._update_progress(
                task, stage="下载视频", detail="正在下载 MP4 视频文件..."
            )

            def _on_progress(d: Any) -> None:
                ...
                try:
                    if downloaded is not None and total:
                        pct = int(int(downloaded) * 100 / int(total))
                except Exception:
                    pct = None

                detail = "正在下载 MP4 视频文件..."
                if pct is not None:
                    detail = f"正在下载 MP4 视频文件... {pct}%"

                # "downloading" 期间进度映射到 0-95，完成后再置 100。
                if pct is not None:
                    self._update_progress(
                        task,
                        progress=min(95, max(1, pct)),
                        stage="下载视频",
                        detail=detail,
                    )
                else:
                    self._update_progress(task, stage="下载视频", detail=detail)

            # Download artifacts should be isolated under output/<task_id>/
            ...
            final_path = self.video_downloader.download_video(
                ...
                cookies_str=cookies_str,
                cookies_domain=cookies_domain,
                format_selector=getattr(task, "download_format", None),
                progress_cb=_on_progress,
                no_progress_timeout_sec=120,
            )

            task.video_file_path = final_path
            task.progress = 100
            ...
            task.status = "failed"
            task.error_message = str(e) or "下载已取消"
            task.progress_stage = "下载视频"
            task.progress_detail = "下载失败"
            self.logger.warning(f"[{task_id}] 下载取消: {e}")
            ...
            task.progress_stage = "下载视频"
            task.progress_detail = "下载失败"
            self.logger.error(f"[{task_id}] 下载失败: {e}")
    ...
    def _step_process_audio_and_transcribe(
        ...
        if audio_info["duration"] > self.long_audio_threshold:
            try:
                self.logger.debug(
                    f"[DEBUG] 音频超过阈值({self.long_audio_threshold}s)，开始分段处理"
                )
                segments = self.audio_extractor.split_audio_by_duration(
                    audio_path, self.segment_duration
                )
                self.logger.debug(
                    f"[DEBUG] 音频分段完成，共 {len(segments)} 个片段（每段 {self.segment_duration}s）"
                )
                task.progress = 40
            ...
            for i, segment in enumerate(segments):
                ...
                task.progress_detail = (
                    f"正在处理第 {i + 1}/{len(segments)} 个音频片段..."
                )
                ...
                try:
                    ...
                    self.logger.info(
                        f"[{task_id}] 片段 {i + 1} 处理成功: 文本长度={len(text)}"
                    )
                    consecutive_failures = 0
                    ...
                    self.logger.warning(
                        f"[{task_id}] 片段 {i + 1} 处理失败: {e} (连续失败次数: {consecutive_failures})"
                    )
                    transcription_results.append(
                        {
                            "segment_index": segment["index"],
                    ...
                    if consecutive_failures >= max_consecutive_failures:
                        self.logger.warning(
                            f"[{task_id}] 连续失败次数达到上限 ({max_consecutive_failures})，停止处理"
                        )
                        remaining_segments = len(segments) - (i + 1)
                        if remaining_segments > 0:
                            self.logger.info(
                                f"[{task_id}] 跳过剩余 {remaining_segments} 个片段"
                            )
                        break
            ...
            # 注册所有分段文件，便于统一清理
            try:
                self.file_uploader.file_manager.register_task(
                    task_id, [s["path"] for s in segments]
                )
            ...
            for result in transcription_results:
                if not result.get("error"):
                    ...
                    if text and len(text) >= 3:
                        ...
                        self.logger.warning(
                            f"[{task_id}] 警告: 片段 {result.get('segment_index', '未知')} 返回无效文本"
                        )
                    ...
                    failed_segments += 1
                    self.logger.warning(
                        f"[{task_id}] 片段 {result.get('segment_index', '未知')} 处理失败: {result.get('error', '未知错误')}"
                    )

            self.logger.info(
                f"[{task_id}] 处理统计: 成功 {successful_segments}/{processed_segments_count} 个片段, 失败 {failed_segments} 个片段"
            )
            if processed_segments_count < len(segments):
                remaining_segments = len(segments) - processed_segments_count
                self.logger.info(
                    f"[{task_id}] 跳过未处理片段: {remaining_segments} 个 (因连续失败而提前终止)"
                )
            if successful_segments == 0:
                raise Exception(
                    f"所有音频片段处理失败，共处理 {processed_segments_count} 个片段"
                )
                ...
                try:
                    ...
                    if not text or len(text) < 3:
                        if retry < max_retries - 1:
                            self.logger.warning(
                                f"[{task_id}] 短音频返回无效文本（长度={len(text)}），第 {retry + 1} 次重试..."
                            )
                            time.sleep(self.retry_sleep_long)
                            continue
                        else:
                            raise Exception(
                                f"短音频处理重试{max_retries}次后仍返回无效文本"
                            )
                    self.logger.info(
                        f"[{task_id}] 短音频处理成功: 文本长度={len(text)}"
                    )
                    break
                except Exception as e:
                    if retry < max_retries - 1:
                        self.logger.warning(
                            f"[{task_id}] 短音频处理失败: {e}，第 {retry + 1} 次重试..."
                        )
                        time.sleep(self.retry_sleep_long)
                    else:
    ...
    def process_video(
        self, task_id: str, llm_provider: str = None, api_config: dict = None
    ) -> ProcessingTask:
        """处理视频的完整流程"""
        task = self.get_task(task_id)
        if not task:
        ...
        try:
            ...
            if full_text is None:
                return task
            self.logger.info(
                f"[{task_id}] 合并转录结果: 文本长度={len(full_text)}, 片段数={len(all_segments)}"
            )
    ...
    def process_upload(
        ...
        self.logger.info(
            f"[{task_id}] 任务状态: status={task.status}, upload_status={task.upload_status}"
        )
        self.logger.info(
        ...
        try:
            ...
            task.progress = 10
            task.progress_detail = (
                f"文件大小: {file_info.get('file_size', 0) // (1024 * 1024)}MB"
            )
            task.estimated_time = int(
                file_info.get("duration", 300) * 0.3
            )  # 粗略估计处理时间
            ...
            if task.need_audio_extraction:
                ...
                try:
                    ...
                    try:
                        ...
                        self.logger.warning(
                            f"[{task_id}] 删除原始视频文件失败: {delete_error}"
                        )
                    ...
                    try:
                        self.file_uploader.file_manager.register_task(
                            task_id, [audio_path], register_dir=True
                        )
                    except Exception as reg_err:
            ...
            try:
                if self._is_cancelled(task_id):
                    ...
                    return task
                self.logger.debug(
                    f"[{task_id}] 开始获取音频信息: {task.audio_file_path}"
                )
    ...
    def _save_results(self, task: ProcessingTask):
        ...
        try:
            with open(transcript_path, "w", encoding="utf-8") as f:
                f.write(
                    f"# {task.video_info.title if task.video_info else '视频逐字稿'}\n\n"
                )
                url_line = (
                    task.video_info.url
        ...
        try:
            with open(summary_path, "w", encoding="utf-8") as f:
                f.write(
                    f"# {task.video_info.title if task.video_info else '视频总结报告'}\n\n"
                )
                url_line = (
                    task.video_info.url
                    if task.video_info and getattr(task.video_info, "url", "")
                ...
                if task.video_info:
                    ...
                    f.write(
                        f"**时长:** {self._format_duration(task.video_info.duration)}\n"
                    )
                f.write(
                    f"**处理时间:** {task.created_at.strftime('%Y-%m-%d %H:%M:%S')}\n\n"
                )
    ...
    def get_task_progress(self, task_id: str) -> Dict[str, Any]:
        ...
        if task.transcript and task.transcript.strip():
            progress_info["transcript_preview"] = (
                task.transcript[:500] + "..."
                if len(task.transcript) > 500
                else task.transcript
            )
            progress_info["full_transcript"] = task.transcript
            progress_info["transcript_ready"] = True
        elif (
            task.transcription and getattr(task.transcription, "full_text", "").strip()
        ):
        ...
        try:
            if os.path.exists(self.tasks_file):
                ...
                if len(completed_tasks) != len(tasks_data):
                    ...
                    self.logger.info(
                        f"已清理未完成任务，加载 {len(self.tasks)} 个历史任务"
                    )
                else:
    ...
    def _smart_cleanup_temp_files(self, current_task_id: str, current_audio_path: str):
        ...
        try:
            ...
            self.logger.info(
                f"[{current_task_id}] 已触发历史任务清理（按最近{self.file_uploader.file_manager.max_temp_tasks}个保留）"
            )
        except Exception as e:
